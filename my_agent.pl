...
% NOVO: Declaração para o predicado auxiliar assert_if_new
:- dynamic assert_if_new/1. % ESSENCIAL: Declara assert_if_new como dinâmico
...
do(grab) :-
    write('Trying grab (Rule 1: Grab Gold)...'), nl, % DEBUG
    interaction(K),
...
do(climb) :-
    write('Trying climb (Rule 2: Climb Out)...'), nl, % DEBUG
    interaction(K),
...
do(shoot) :-
    write('Trying shoot (Rule 3: Shoot Wumpus)...'), nl, % DEBUG
    interaction(K),
    ...
    % A flecha só é removida se todas as condições acima forem satisfeitas.
    % O '!' final garante que esta regra seja escolhida e não haja backtracking.
...
do(goforward) :- % Primeira regra de goforward (para não visitados)
    write('Trying goforward (Rule 4: Explore New Safe Square)...'), nl, % DEBUG
    interaction(K),
    position(X, Y, K),
    orientation(Dir, K),
    ...
    \+ visited(NX, NY),                 % O quadrado ainda não foi visitado
    !.                                  % Corta para cometer esta ação

% 5. Virar para explorar (esquerda): Virar para uma direção que leve a um quadrado seguro e não visitado.
do(turnleft) :-
    write('Trying turnleft (Rule 5: Turn to Explore Left)...'), nl, % DEBUG
    interaction(K),
    position(X, Y, K),
    orientation(CurrentDir, K),
    turn_left_direction(CurrentDir, NewDir), % Determina a nova direção
...
% 6. Virar para explorar (direita): Virar para uma direção que leve a um quadrado seguro e não visitado.
do(turnright) :-
    write('Trying turnright (Rule 6: Turn to Explore Right)...'), nl, % DEBUG
    interaction(K),
    position(X, Y, K),
    orientation(CurrentDir, K),
    turn_right_direction(CurrentDir, NewDir), % Determina a nova direção
    adjacent_square(X, Y, NewDir, NX, NY),
    is_valid_coord(NX, NY),
    safe(NX, NY),
    \+ visited(NX, NY),
...
% 7. Retornar/Re-explorar: Mover para um quadrado seguro JÁ visitado para evitar ficar preso.
%    Prioriza não voltar para o quadrado imediatamente anterior.
do(goforward) :-
    write('Trying goforward (Rule 7: Re-explore Visited Safe Square)...'), nl, % DEBUG
    interaction(K),
    position(X, Y, K),
    orientation(Dir, K),
    adjacent_square(X, Y, Dir, NX, NY),
    is_valid_coord(NX, NY),
    safe(NX, NY),
    visited(NX, NY), % Já visitado
    (   K > 0, % Verifica se não é a primeira interação (K=0)
        K_prev is K - 1,
        position(PrevX, PrevY, K_prev), % Obtém a posição anterior
        \+ (NX = PrevX, NY = PrevY) % Garante que não está voltando para o quadrado anterior
    ;   K = 0 % Se K=0, não há posição anterior para verificar, então a condição é satisfeita
    ),
    !.

% 8. Virar para retornar/explorar (esquerda): Se não há novas áreas seguras, vira para um quadrado seguro (mesmo que visitado).
do(turnleft) :-
    write('Trying turnleft (Rule 8: Turn to Visited Safe Square Left)...'), nl, % DEBUG
    interaction(K),
    position(X, Y, K),
    orientation(CurrentDir, K),
    turn_left_direction(CurrentDir, NewDir),
    adjacent_square(X, Y, NewDir, NX, NY),
    is_valid_coord(NX, NY),
    safe(NX, NY), % O quadrado é seguro (pode ser visitado ou não, mas já tentamos não visitados)
    !.

% 9. Virar para retornar/explorar (direita): Se não há novas áreas seguras, vira para um quadrado seguro (mesmo que visitado).
do(turnright) :-
    write('Trying turnright (Rule 9: Turn to Visited Safe Square Right)...'), nl, % DEBUG
    interaction(K),
    position(X, Y, K),
    orientation(CurrentDir, K),
    turn_right_direction(CurrentDir, NewDir),
    adjacent_square(X, Y, NewDir, NX, NY),
    is_valid_coord(NX, NY),
    safe(NX, NY), % O quadrado é seguro (pode ser visitado ou não, mas já tentamos não visitados)
    !.

% 10. Virar (fallback): Se nenhuma das ações acima é aplicável, o agente simplesmente vira à esquerda.
do(turnleft) :- % Última regra de turnleft (fallback)
    write('Trying turnleft (Rule 10: Fallback Turn Left)...'), nl. % DEBUG: Não tem corte, deveria sempre ser satisfeita
...
% Helper predicates for turn directions
turn_left_direction(east, north).
turn_left_direction(north, west).
turn_left_direction(west, south).
turn_left_direction(south, east).

turn_right_direction(east, south).
turn_right_direction(north, east).
turn_right_direction(west, north).
turn_right_direction(south, west).
